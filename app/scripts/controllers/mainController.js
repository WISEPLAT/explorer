...
angular.module('wshExplorer')
    .controller('mainCtrl', function ($rootScope, $scope, $location) {
        ...
        web3.wsh.filter("latest", function(error, result){
          if (!error) {
            getWSHRates();
            updateBlockList();
            updateTXList();
        ...
        $scope.processRequest= function(){
            var requestStr = $scope.wshRequest;
            ...
            if (requestStr!==undefined){
                ...
                var regexpAddr = /^(0x)?[0-9a-f]{40}$/; //New wsh Regular Expression for Addresses
        ...
        function updateStats() {
          $scope.blockNum = web3.wsh.blockNumber; // now that was easy

          if($scope.blockNum!==undefined){

            // TODO: put the 2 web3.wsh.getBlock into the async function below
            //       easiest to first do with fastInfosCtrl
              var blockNewest = web3.wsh.getBlock($scope.blockNum);
              ...
              if(blockNewest!==undefined){
                ...
                // Time
                  ...
                  var blockBefore = web3.wsh.getBlock($scope.blockNum - 1);
                  if(blockBefore!==undefined){
                  $scope.blocktime = blockNewest.timestamp - blockBefore.timestamp;
                  }
                  $scope.range1=100;
                  range = $scope.range1;
                  var blockPast = web3.wsh.getBlock(Math.max($scope.blockNum - range,0));
                  if(blockBefore!==undefined){
                  ...
                  var blockPast = web3.wsh.getBlock(1);
                  if(blockBefore!==undefined){
          ...
          $scope.versionCurrency = web3.version.wise; // TODO: change that to currencyname?
        ...
        function getWSHRates() {
          $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum/", function(json) {
            var price = Number(json[0].price_usd);
            $scope.wshprice = "$" + price.toFixed(2);
          });

          $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum/", function(json) {
            var btcprice = Number(json[0].price_btc);
            $scope.wshbtcprice = btcprice;
          });

          $.getJSON("https://api.coinmarketcap.com/v1/ticker/ethereum/", function(json) {
            var cap = Number(json[0].market_cap_usd);
            //console.log("Current wsh Market Cap: " + cap);
            $scope.wshmarketcap = cap;
          });
        }

        function updateTXList() {
            var currentTXnumber = web3.wsh.blockNumber;
            $scope.txNumber = currentTXnumber;
            $scope.recenttransactions = [];
            for (var i=0; i < 10 && currentTXnumber - i >= 0; i++) {
              $scope.recenttransactions.push(web3.wsh.getTransactionFromBlock(currentTXnumber - i));
            }
        }

        function updateBlockList() {
            var currentBlockNumber = web3.wsh.blockNumber;
            $scope.blockNumber = currentBlockNumber;
            $scope.blocks = [];
            for (var i=0; i < 10 && currentBlockNumber - i >= 0; i++) {
              $scope.blocks.push(web3.wsh.getBlock(currentBlockNumber - i));
            }
        }
...
angular.module('filters', []).
  ...
  filter('BigNum', function () {
    return function (txt) {
      if (isNaN(txt)) return txt;
      var b = new BigNumber(txt);
      var w = web3.fromWei(b, "wise");
      return w.toFixed(6) + " WSH";
    };
  }).
