...
/*
    This file is part of wiseplat.js.

    wiseplat.js is free software: you can redistribute it and/or modify
    it under the terms of the GNU Lesser General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    wiseplat.js is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU Lesser General Public License for more details.

    You should have received a copy of the GNU Lesser General Public License
    along with wiseplat.js.  If not, see <http://www.gnu.org/licenses/>.
*/
/** 
 * @file coder.js
...
var formatInputInt = function (value) {
    BigNumber.config(c.WSH_BIGNUMBER_ROUNDING_MODE);
...
module.exports = function (str, isNew) {
    if (str.substr(0, 2) === '0x' && !isNew) {
        ...
        console.warn('see https://github.com/wiseplat/web3.js/pull/205');
...
var unitMap = {
    ...
    'nano':         '1000000000',
    'szabo':        '1000000000000',
    'microwise':   '1000000000000',
    'micro':        '1000000000000',
    'finney':       '1000000000000000',
    'milliwise':    '1000000000000000',
    'milli':         '1000000000000000',
    'wise':        '1000000000000000000',
    'kwise':       '1000000000000000000000',
    'grand':        '1000000000000000000000',
    'einstein':     '1000000000000000000000',
    'mwise':       '1000000000000000000000000',
    'gwise':       '1000000000000000000000000000',
    'twise':       '1000000000000000000000000000000'
};
 ...
 * @param {String} unit the unit to convert to, default wise
 * @returns {BigNumber} value of the unit (in Wei)
 * @throws error if the unit is not correct:w
 */
var getValueOfUnit = function (unit) {
    unit = unit ? unit.toLowerCase() : 'wise';
...
/**
 * Takes a number of wei and converts it to any other wise unit.
 *
 * Possible units are:
 *   SI Short   SI Full        Effigy       Other
 * - kwei       femtowise     ada
 * - mwei       picowise      babbage
 * - gwei       nanowise      shannon      nano
 * - --         microwise     szabo        micro
 * - --         milliwise     finney       milli
 * - wise      --             --
 * - kwise                    einstein     grand
 * - mwise
 * - gwise
 * - twise
 *
 ...
 * @param {String} unit the unit to convert from, default wise
 * @return {String|Object} When given a BigNumber object it returns one as well, otherwise a number
*/
...
var wsh = require('./web3/methods/wsh');
...
/*jshint maxparams:4 */
web3.wsh.filter = function (fil, callback) {
    return new Filter(fil, watches.wsh(), formatters.outputLogFormatter, callback);
};
/*jshint maxparams:3 */
...
// ADD defaultblock
Object.defineProperty(web3.wsh, 'defaultBlock', {
    get: function () {
...
Object.defineProperty(web3.wsh, 'defaultAccount', {
    get: function () {
...
setupMethods(web3.wsh, wsh.methods);
setupProperties(web3.wsh, wsh.properties);
...
AllSolidityEvents.prototype.execute = function (options, callback) {
    ...
    return new Filter(o, watches.wsh(), formatter, callback);
};
...
var checkForContractAddress = function(contract, abi, callback){
    ...
    // wait for receipt
    var filter = web3.wsh.filter('latest', function(e){
        if(!e && !callbackFired) {
            count++;
            ...
            if(count > 50) {
                ...
                web3.wsh.getTransactionReceipt(contract.transactionHash, function(e, receipt){
                    if(receipt && !callbackFired) {

                        web3.wsh.getCode(receipt.contractAddress, function(e, code){
                            /*jshint maxcomplexity: 5 */
...
ContractFactory.prototype.new = function () {
    ...
    if(callback) {
        ...
        web3.wsh.sendTransaction(options, function (err, hash) {
            if (err) {
                callback(err);
            } else {
        ...
        var hash = web3.wsh.sendTransaction(options);
        // add the transaction hash
        contract.transactionHash = hash;
...
SolidityFunction.prototype.call = function () {
    ...
    if (!callback) {
        var output = web3.wsh.call(payload, defaultBlock);
        return this.unpackOutput(output);
    } 
        
    var self = this;
    web3.wsh.call(payload, defaultBlock, function (error, output) {
...
SolidityFunction.prototype.sendTransaction = function () {
    ...
    if (!callback) {
        return web3.wsh.sendTransaction(payload);
    }

    web3.wsh.sendTransaction(payload, callback);
};
...
SolidityFunction.prototype.estimateGas = function () {
    ...
    if (!callback) {
        return web3.wsh.estimateGas(payload);
    }

    web3.wsh.estimateGas(payload, callback);
};
...
SolidityFunction.prototype.request = function () {
    ...
    return {
        method: this._constant ? 'wsh_call' : 'wsh_sendTransaction',
        callback: callback,
        params: [payload], 
        format: format
    };
...
/**
 * This method should be used to create iban object from wiseplat address
 *
 * @method fromAddress
 * @param {String} address
...
Iban.createIndirect = function (options) {
    return Iban.fromBban('WSH' + options.institution + options.identifier);
};
...
Iban.prototype.isValid = function () {
    return /^XE[0-9]{2}(WSH[0-9A-Z]{13}|[0-9A-Z]{30,31})$/.test(this._iban) &&
...
var blockCall = function (args) {
    return (utils.isString(args[0]) && args[0].indexOf('0x') === 0) ? "wsh_getBlockByHash" : "wsh_getBlockByNumber";
};

var transactionFromBlockCall = function (args) {
    return (utils.isString(args[0]) && args[0].indexOf('0x') === 0) ? 'wsh_getTransactionByBlockHashAndIndex' : 'wsh_getTransactionByBlockNumberAndIndex';
};

var uncleCall = function (args) {
    return (utils.isString(args[0]) && args[0].indexOf('0x') === 0) ? 'wsh_getUncleByBlockHashAndIndex' : 'wsh_getUncleByBlockNumberAndIndex';
};

var getBlockTransactionCountCall = function (args) {
    return (utils.isString(args[0]) && args[0].indexOf('0x') === 0) ? 'wsh_getBlockTransactionCountByHash' : 'wsh_getBlockTransactionCountByNumber';
};

var uncleCountCall = function (args) {
    return (utils.isString(args[0]) && args[0].indexOf('0x') === 0) ? 'wsh_getUncleCountByBlockHash' : 'wsh_getUncleCountByBlockNumber';
};

/// @returns an array of objects describing web3.wsh api methods
...
var getTransactionReceipt = new Method({
    ...
    call: 'wsh_getTransactionReceipt',
    params: 1,
...
/// @returns an array of objects describing web3.wsh api properties
...
/// @returns an array of objects describing web3.wsh.filter api methods
var wsh = function () {
    var newFilterCall = function (args) {
        ...
        switch(type) {
            case 'latest':
                args.shift();
                this.params = 0;
                return 'wsh_newBlockFilter';
            case 'pending':
                args.shift();
                this.params = 0;
                return 'wsh_newPendingTransactionFilter';
            default:
                return 'wsh_newFilter';
        }
    };
    ...
    var uninstallFilter = new Method({
        ...
        call: 'wsh_uninstallFilter',
        params: 1
    });

    var getLogs = new Method({
        name: 'getLogs',
        call: 'wsh_getFilterLogs',
        params: 1
    });

    var poll = new Method({
        name: 'poll',
        call: 'wsh_getFilterChanges',
        params: 1
    });
 ...
 * It's also responsible for polling the wiseplat node for incoming messages
...
RequestManager.prototype.poll = function () {
    /*jshint maxcomplexity: 6 */
    this.timeout = setTimeout(this.poll.bind(this), c.WSH_POLLING_TIMEOUT);
...
var transferToAddress = function (from, to, value, callback) {
    return web3.wsh.sendTransaction({
        address: to,
        from: from,
        value: value
    }, callback);
...
web3.wsh.contract = require('./lib/web3/contract');
web3.wsh.namereg = namereg.namereg;
web3.wsh.ibanNamereg = namereg.ibanNamereg;
web3.wsh.sendIBANTransaction = require('./lib/web3/transfer');
web3.wsh.iban = require('./lib/web3/iban');
